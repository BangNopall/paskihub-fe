...
*(Note for agent: When executing, replace the specific `<form>` block with shadcn `<Form>` components, use `useForm<LoginFormData>({ resolver: zodResolver(loginFormSchema) })`, and call `signIn("credentials", { ...data, redirect: false })` on submit. Show toast on success/error).*
...
*(Remaining pages like Register, Forgot Password will be handled similarly by the executing agent by binding the RHF to the server actions created in Task 3).*
