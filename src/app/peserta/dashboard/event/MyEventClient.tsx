...
import { registerEventAction, pelunasanEventAction } from "@/actions/participant-event.actions"
...
function PaymentBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    DP_PAID: "bg-red-50 border-red-300 text-red-500",
  ...
  const badgeClass = styles[status] || "bg-gray-50 border-gray-400 text-gray-600"
...
export default function MyEventClient({ initialOpenEvents, initialActiveEvents, myTeams }: MyEventClientProps) {
  ...
  const [selectedEventToRegister, setSelectedEventToRegister] = useState<OpenEvent | null>(null)
  ...
  const handleReupload = (activeEvent: ActiveEvent) => {
    const matchedEvent = initialOpenEvents.find((e) => e.name === activeEvent.event_name)
  ...
  const submitReupload = async (e: React.FormEvent) => {
    ...
    const res = await pelunasanEventAction(reuploadActiveEvent.registration_id, formData)
  ...
  const exploreEvents = initialOpenEvents.filter(ev => {
    ...
    if (locationFilter !== "all" && ev.location && !ev.location.toLowerCase().includes(locationFilter.toLowerCase())) {
      return false
    }
    return true
  })
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 p-4 md:gap-10 md:p-6 lg:p-8">
        ...
        <Tabs defaultValue="active" className="flex w-full flex-col gap-6 md:gap-8">
          ...
          <TabsContent value="explore" className="m-0 flex flex-col gap-6 outline-none">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-stone-200">
                          {event.logo_path ? (
                            <img
                              src={event.logo_path.startsWith("/") ? process.env.NEXT_PUBLIC_API_URL + event.logo_path : event.logo_path}
                              alt="Logo"
            ...
            <div className="flex flex-col gap-4 rounded-3xl border border-sky-100 bg-gradient-to-b from-white/60 to-white/40 p-4 shadow-sm backdrop-blur-sm md:p-6">
                ...
                <Table>
                  <TableHeader className="bg-blue-50/50">
                    <TableRow className="border-sky-100 hover:bg-transparent">
                      <TableHead className="w-20 py-4 text-center font-poppins text-sm font-semibold text-neutral-700">Logo</TableHead>
                      <TableHead className="py-4 text-center font-poppins text-sm font-semibold text-neutral-700">Nama Event</TableHead>
                      <TableHead className="py-4 text-center font-poppins text-sm font-semibold text-neutral-700">Tim</TableHead>
                      <TableHead className="py-4 text-center font-poppins text-sm font-semibold text-neutral-700">Status Pembayaran</TableHead>
                  ...
                  <TableBody>
                    {initialActiveEvents.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={5} className="py-8 text-center font-poppins text-neutral-500">
                      ...
                      initialActiveEvents.map((event) => (
                        <React.Fragment key={event.registration_id}>
                          <TableRow className={`border-sky-100 bg-transparent hover:bg-white/50 ${event.payment_status === "REJECTED" ? "border-b-0" : ""}`}>
                            <TableCell className="p-4">
                              <div className="mx-auto flex h-10 w-10 items-center justify-center overflow-hidden rounded-lg bg-stone-200">
                                {event.event_logo_path ? (
                                  <img
                                    src={event.event_logo_path.startsWith("/") ? process.env.NEXT_PUBLIC_API_URL + event.event_logo_path : event.event_logo_path}
                                    alt="Logo"
                                    className="h-full w-full object-cover"
                                  />
                                ) : (
                            ...
                            <TableCell className="p-4">
                              <div className="flex justify-center gap-2">
                                {(event.payment_status === "FULL_PAID" || event.payment_status === "DP_PAID") && (
                                  <Button
                                    onClick={() => handleDashboardAction(event.registration_id)}
                                ...
                                {event.payment_status === "WAITING" && (
                                  <span className="font-poppins text-sm font-bold text-neutral-400">-</span>
                                )}
                              </div>
                            </TableCell>
                          </TableRow>
              ...
              <div className="flex flex-col gap-4 md:hidden">
                {initialActiveEvents.length === 0 ? (
                  ...
                  initialActiveEvents.map((event) => (
                    <div key={event.registration_id} className="flex flex-col gap-4 rounded-xl border border-sky-100 bg-white p-4 shadow-sm">
                      <div className="flex items-center gap-3">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-stone-200">
                          {event.event_logo_path ? (
                            <img src={event.event_logo_path.startsWith("/") ? process.env.NEXT_PUBLIC_API_URL + event.event_logo_path : event.event_logo_path} alt="Logo" className="h-full w-full object-cover" />
                          ) : (
                            <ImageIcon className="h-5 w-5 text-neutral-400" />
                          )}
                        </div>
                        <div className="flex flex-col">
                          <h3 className="font-poppins text-sm font-semibold text-neutral-800">{event.event_name}</h3>
                          <p className="font-poppins text-xs font-medium text-neutral-500">{event.team_name}</p>
                        </div>
                      </div>
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="pt-2">
                          {(event.payment_status === "FULL_PAID" || event.payment_status === "DP_PAID") && (
                            <Button onClick={() => handleDashboardAction(event.registration_id)} className="h-10 w-full rounded-lg bg-blue-500 text-sm font-semibold text-white hover:bg-blue-600">
                              Dashboard Event
                            </Button>
                          )}
                          {event.payment_status === "REJECTED" && (
                            <Button onClick={() => handleReupload(event)} className="h-10 w-full rounded-lg bg-red-500 text-sm font-semibold text-white hover:bg-red-600">
                              Upload Ulang Bukti
                            </Button>
                          )}
                          {event.payment_status === "WAITING" && (
                            <div className="flex h-10 w-full items-center justify-center rounded-lg bg-neutral-100">
                              <span className="font-poppins text-xs font-medium text-neutral-500">Menunggu Tindakan Admin</span>
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <form onSubmit={submitRegistration} className="flex flex-col">
                ...
                <div className="flex flex-col gap-6 p-6 sm:px-10 sm:pt-6 sm:pb-10">
                  <div className="flex flex-col gap-4 rounded-2xl border border-sky-100 bg-sky-50 p-4 sm:p-5">
                    <div className="flex items-center gap-4">
                      <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-stone-300">
                        {selectedEventToRegister.logo_path ? (
                          <img src={selectedEventToRegister.logo_path.startsWith("/") ? process.env.NEXT_PUBLIC_API_URL + selectedEventToRegister.logo_path : selectedEventToRegister.logo_path} alt="Logo" className="h-full w-full object-cover" />
                        ) : (
                    ...
                    <p className="font-poppins text-sm text-neutral-500">{selectedEventToRegister.description}</p>

                    <div className="grid grid-cols-2 gap-4 pt-2 sm:gap-6">
                      <div className="flex flex-col gap-1">
                        <span className="font-poppins text-xs text-neutral-500 sm:text-sm">Tanggal:</span>
                        <span className="font-poppins text-sm font-medium text-neutral-800">{selectedEventToRegister.open_date} - {selectedEventToRegister.close_date}</span>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="font-poppins text-xs text-neutral-500 sm:text-sm">Lokasi:</span>
                        <span className="font-poppins text-sm font-medium text-neutral-800">{selectedEventToRegister.location}</span>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="font-poppins text-xs text-neutral-500 sm:text-sm">Anggota per Tim:</span>
                        <span className="font-poppins text-sm font-medium text-neutral-800">{selectedEventToRegister.min_team_members}-{selectedEventToRegister.max_team_members}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="mt-1 font-poppins text-xs text-neutral-400">
                      Jika belum buat tim, <Link href="/peserta/dashboard/team/new" className="font-medium text-blue-500 hover:underline">klik di sini untuk buat tim</Link>
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <RadioGroup value={paymentType} onValueChange={(v) => setPaymentType(v as "lunas" | "dp")} className="flex flex-col gap-3">
                      <Label htmlFor="pay-lunas" className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition-colors ${paymentType === "lunas" ? "border-blue-500 bg-blue-50/50" : "border-neutral-200 bg-white hover:bg-neutral-50"}`}>
                        <RadioGroupItem value="lunas" id="pay-lunas" className="mt-0.5 border-neutral-300 text-blue-500" />
                        <div className="flex flex-col gap-1">
                      ...
                      <Label htmlFor="pay-dp" className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition-colors ${paymentType === "dp" ? "border-blue-500 bg-blue-50/50" : "border-neutral-200 bg-white hover:bg-neutral-50"}`}>
                        <RadioGroupItem value="dp" id="pay-dp" className="mt-0.5 border-neutral-300 text-blue-500" />
                        <div className="flex flex-col gap-1">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="file" className="hidden" ref={fileInputRef} accept=".jpg,.jpeg,.png,.pdf" onChange={handleFileChange} />
                  ...
                  <div className="mt-4 flex flex-col-reverse items-center gap-3 sm:flex-row">
                    <Button type="button" variant="outline" onClick={handleCloseRegistrationModal} className="h-12 w-full rounded-full border-neutral-300 font-poppins text-base font-semibold text-neutral-600 hover:bg-neutral-100 sm:flex-1">Batal</Button>
                    <Button type="submit" disabled={isSubmitting || !selectedTeamId || !paymentProof} className="h-12 w-full rounded-full bg-red-400 font-poppins text-base font-bold text-white shadow-sm hover:bg-red-500 disabled:opacity-50 sm:flex-1">
                      {isSubmitting ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Mendaftar...</> : "Daftar Event"}
                    </Button>
                  </div>
                </div>
              </form>
          ...
          <DialogContent className="max-h-[90vh] w-full max-w-2xl gap-0 overflow-y-auto rounded-xl p-0 sm:rounded-3xl">
            {selectedEventToRegister && reuploadActiveEvent && (
              <form onSubmit={submitReupload} className="flex flex-col">
                <DialogHeader className="border-b border-neutral-200 p-6 pb-4 sm:px-10 sm:pt-10">
                  <DialogTitle className="text-center font-montserrat text-2xl font-bold text-neutral-800">Upload Ulang Bukti</DialogTitle>
                </DialogHeader>
                ...
                <div className="flex flex-col gap-6 p-6 sm:px-10 sm:pt-6 sm:pb-10">
                  <div className="flex flex-col gap-4 rounded-2xl border border-sky-100 bg-sky-50 p-4 sm:p-5">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="font-poppins text-base font-semibold text-neutral-800 sm:text-lg">{selectedEventToRegister.name}</h3>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6">
                    <div className="flex flex-col gap-2">
                      <Label className="font-poppins text-sm font-normal text-neutral-700">Tim Anda</Label>
                  ...
                  <div className="flex flex-col gap-2">
                    <Label className="font-poppins text-sm font-normal text-neutral-700">Upload Bukti Pembayaran <span className="text-red-500">*</span></Label>
                    <input type="file" className="hidden" ref={reuploadFileInputRef} accept=".jpg,.jpeg,.png,.pdf" onChange={handleReuploadFileChange} />
                    <div onClick={() => reuploadFileInputRef.current?.click()} className="group flex cursor-pointer flex-col items-center justify-center gap-4 rounded-xl border border-dashed border-neutral-300 bg-neutral-50 p-6 transition-all hover:border-blue-400 hover:bg-blue-50/50 sm:p-8">
                      {reuploadProof ? (
                        <>
                          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-100 text-blue-600"><FileText className="h-6 w-6" /></div>
                          <div className="text-center">
                            <p className="max-w-[200px] truncate font-poppins text-sm font-semibold text-neutral-800 sm:max-w-xs">{reuploadProof.name}</p>
                            <p className="mt-1 font-poppins text-xs text-neutral-500">Klik untuk mengganti file</p>
                          </div>
                        </>
                      ) : (
                  ...
                  <div className="mt-4 flex flex-col-reverse items-center gap-3 sm:flex-row">
                    <Button type="button" variant="outline" onClick={handleCloseReuploadModal} className="h-12 w-full rounded-full border-neutral-300 font-poppins text-base font-semibold text-neutral-600 hover:bg-neutral-100 sm:flex-1">Batal</Button>
                    <Button type="submit" disabled={isReuploading || !reuploadProof} className="h-12 w-full rounded-full bg-red-400 font-poppins text-base font-bold text-white shadow-sm hover:bg-red-500 disabled:opacity-50 sm:flex-1">
                      {isReuploading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Uploading...</> : "Upload Bukti"}
                    </Button>
                  </div>
                </div>
              </form>
