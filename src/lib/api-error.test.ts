...
describe("parseApiError", () => {
  it("throws ApiError instead of returning empty object", async () => {
    const mockRes = new Response(JSON.stringify({ message: "Not Found" }), {
      status: 404,
    })
  ...
  it("handles KICKED specially", async () => {
    const mockRes = new Response(JSON.stringify({ status: "KICKED" }), {
      status: 400,
    })
  ...
  it("throws 500 server error safely without crashing", async () => {
    ...
    await expect(parseApiError(mockRes)).rejects.toThrow(
