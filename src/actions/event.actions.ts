...
export async function updateEventAction(id: string, data: any) {
  try {
    ...
    return { success: false, message: error.message || "Gagal memperbarui event" };
  }
}
...
export async function uploadEventPosterAction(id: string, formData: FormData) {
  try {
    ...
    return { success: false, message: error.message || "Gagal mengunggah poster" };
  }
}
...
export async function createEventLevelAction(eventId: string, data: any) {
  try {
    ...
    return { success: false, message: error.message || "Gagal menambahkan jenjang" };
  }
}

export async function updateEventLevelAction(eventId: string, levelId: string, data: any) {
  try {
    const session: any = await getServerSession(authOptions);
    if (!session?.accessToken) throw new Error("Unauthorized");

    await profileService.updateEventLevel(eventId, levelId, data, session.accessToken);
    revalidatePath("/organizer/dashboard/event");
    return { success: true, message: "Jenjang berhasil diperbarui." };
  } catch (error: any) {
    return { success: false, message: error.message || "Gagal memperbarui jenjang" };
  }
}
...
export async function deleteEventLevelAction(eventId: string, levelId: string) {
  try {
    ...
    return { success: false, message: error.message || "Gagal menghapus jenjang" };
  }
}
